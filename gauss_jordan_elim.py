...
class GaussJordan_Eliminate(gauss):
    ...
    def jordan_main(self):
        ...
        # if not, make it so. 
        # This does not affect non-pivots are their value is 0
        for row in range(self.rows):
            if not self.matrix.compare_frac(row,row,1):
                self.matrix.make_first_one(row,self.matrix.get_item(row,row))
        return self.matrix
class GaussFailedException(Exception):
    def __init__(self,matrix):
        #super(gauss,matrix)
        pass
    
if __name__ == "__main__":
    g = GaussJordan_Eliminate([[3,-1,7,1],[5,0,1,2]])
    print(g.jordan_main())
...
# Fail: GaussJordan_Eliminate([[3,-1,7,1],[5,0,1,2]]) #System 7
